...
<div>
	<h1 class="text-3xl sm:text-4xl font-bold text-text-dark dark:text-white mb-2">{book.title}</h1>
	...
	<div class="flex items-center justify-between mb-6">
		<p class="text-3xl font-bold text-text-dark dark:text-white">${book.price_in_pound}</p>
		<StockStatus bookAvailable={book.available} />
	</div>
	<div class="flex flex-col sm:flex-row gap-4 mb-6 items-center">
		<Quantity bind:value={amountBook} available={book.available} />
		<button
			class="flex-1 bg-accent hover:bg-accent/90 text-white font-bold py-2 px-6 rounded-lg shadow-md transition-transform transform hover:scale-105"
		>
			Add to Cart
		</button>
		<button
			class="p-2 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 text-text-dark dark:text-gray-300"
		>
